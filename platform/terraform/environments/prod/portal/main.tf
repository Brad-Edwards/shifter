...
module "ssm" {
  ...
  # ACES default cutover (#1310, ADR-031-R6): capability gate + source-route selector.
  shifter_aces_native_provisioning = var.shifter_aces_native_provisioning
  shifter_aces_catalog_cutovers    = var.shifter_aces_catalog_cutovers
...
# never the whole public tier: standalone public workloads (e.g. CTFd in dev)
# live in the separate public-workload tier and so cannot reach Django:8000 /
