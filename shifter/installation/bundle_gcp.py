...
# The Terraform-derived GCP runtime env is authored by the backend renderer, while capacity
# keys are projected by the selected immutable capacity profile during Helm-value assembly.
# Both key sets are owned by ``runtime_inventory_gcp`` so the published bundle can describe
...
def _gcp_capacity_runtime_output(name: str) -> GeneratedOutput:
    ...
    return GeneratedOutput(
        name=name,
        kind=OutputKind.RUNTIME_ENV,
        owner=_GCP_CAPACITY_RENDERER,
        source="selected immutable capacity profile projected during GCP Helm-value assembly",
        destination=OutputDestination.RUNTIME_ENV,
        sensitivity=OutputSensitivity.PUBLIC,
        process_roles=_gcp_output_roles(name),
        description=(
            f"Public GCP shared-service capacity value ({name}) emitted into the platform runtime environment."
        ),
    )
...
def _gcp_generated_outputs() -> tuple[GeneratedOutput, ...]:
    ...
    Built from ``runtime_inventory_gcp``'s renderer and capacity key sets so the contract's
    generated outputs are exactly the runtime keys generated by each producer.
    """
