...
def update_event(event_id: UUID, event_data: dict[str, Any], *, actor_id: int | None = None) -> CTFEvent:
    ...
    with transaction.atomic():
        ...
        publication_changed = (
            "public_registration_enabled" in safe_data
            and safe_data["public_registration_enabled"] != event.public_registration_enabled
        )
        ...
        if publication_changed and actor_id is not None:
            from ctf.services.audit import audit_public_registration_publication
