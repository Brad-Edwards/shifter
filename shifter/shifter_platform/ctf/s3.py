...
def upload_challenge_file(
    ...
    try:
        ...
        logger.error("CTF file upload failed: s3_key=%s error=%s", safe_log_value(s3_key), safe_log_value(e))
...
def delete_challenge_file(s3_key: str) -> None:
    ...
    try:
        ...
        logger.error("CTF file delete failed: s3_key=%s error=%s", safe_log_value(s3_key), safe_log_value(e))
...
def generate_download_url(s3_key: str, filename: str, expires_in: int = 300) -> str:
    ...
    try:
        ...
        logger.error(
            "CTF download URL generation failed: s3_key=%s error=%s", safe_log_value(s3_key), safe_log_value(e)
        )
