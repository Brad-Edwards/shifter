...
def script_upload(request: HttpRequest) -> HttpResponse:
    ...
    """
    logger.info(
        "script_upload: user_id=%s method=%s",
        safe_log_value(request.user.id),
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                logger.exception("script_upload: failed to initiate upload")
...
def script_delete(request: HttpRequest, script_id: int) -> HttpResponse:
    ...
    logger.info(
        "script_delete: user_id=%s script_id=%s",
        safe_log_value(request.user.id),
...
def experiment_create(request: HttpRequest) -> HttpResponse:
    ...
    """
    logger.info(
        "experiment_create: user_id=%s method=%s",
        safe_log_value(request.user.id),
        safe_log_value(request.method),
    )
    if request.method == "GET":
...
def experiment_detail(request: HttpRequest, experiment_id: int) -> HttpResponse:
    ...
    logger.info(
        "experiment_detail: user_id=%s experiment_id=%s",
        safe_log_value(request.user.id),
...
def experiment_start(request: HttpRequest, experiment_id: int) -> HttpResponse:
    ...
    logger.info(
        "experiment_start: user_id=%s experiment_id=%s",
        safe_log_value(request.user.id),
        safe_log_value(experiment_id),
    )
    try:
...
def experiment_cancel(request: HttpRequest, experiment_id: int) -> HttpResponse:
    ...
    logger.info(
        "experiment_cancel: user_id=%s experiment_id=%s",
        safe_log_value(request.user.id),
        safe_log_value(experiment_id),
    )
    try:
...
def experiment_download(request: HttpRequest, experiment_id: int) -> HttpResponse:
    ...
    logger.info(
        "experiment_download: user_id=%s experiment_id=%s",
        safe_log_value(request.user.id),
        safe_log_value(experiment_id),
    )
    try:
...
def scenario_instances(request: HttpRequest, scenario_id: str) -> JsonResponse:
    ...
    logger.info(
        "scenario_instances: user_id=%s scenario_id=%s",
        safe_log_value(request.user.id),
    ...
    try:
        ...
        logger.exception("scenario_instances: validation error")
        return JsonResponse({"error": classify_user_message(e)}, status=400)
    except Exception:
