...
distinct registered ACES package-source id (ADR-031-R6). This module reads the
validated ``settings.ACES_CATALOG_CUTOVERS`` selector (the pure route table) and
...
# ACES catalog entries carry no cyberscript agent requirements.
_ACES_AGENT_REQUIREMENTS = {"requires_windows": False, "requires_linux": False, "has_from_agent": False}
...
def active_cutover_map() -> Mapping[str, str]:
    ...
    validated at settings import (:mod:`config._aces_settings`); this is only the
    read seam so callers never touch ``settings`` directly.
    """
    return getattr(settings, "ACES_CATALOG_CUTOVERS", {}) or {}


def resolve_cutover_source_id(public_id: str) -> str | None:
    """Return the internal ACES source id a public id routes to, or None if unrouted."""
...
def apply_cutover_routes(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    ACES-backed launch choice -- inheriting the target source's launchability and
    ACES fields while keeping its public id, display, and ``ScenarioMetadata``
    ...
    for entry in entries:
        ...
        if source_id is None:
            ...
            continue
        overlaid.append(_overlay_aces_route(entry, by_id.get(source_id)))
    return overlaid


def _overlay_aces_route(public_entry: dict[str, Any], source_entry: dict[str, Any] | None) -> dict[str, Any]:
    """Return the public entry re-backed by its routed ACES source (fail-closed)."""
    overlaid = dict(public_entry)
    overlaid["scenario_type"] = "aces"
    if source_entry is None:
        ...
        overlaid["agent_requirements"] = dict(_ACES_AGENT_REQUIREMENTS)
        return overlaid
    ...
    overlaid["agent_requirements"] = source_entry.get("agent_requirements", dict(_ACES_AGENT_REQUIREMENTS))
    return overlaid
...
class LaunchResolution:
    ...
    ``scenario_id`` is the stable public id to persist/correlate; ``aces_source_id``
    is the internal ACES package-source to load (``None`` for a legacy launch or a
    suppressed target); ``is_aces`` marks the ACES-native path; ``launchable``
    reflects the fail-closed catalog decision.
    """

    scenario_id: str
    aces_source_id: str | None
    is_aces: bool
    launchable: bool
...
def resolve_launch(scenario_id: str) -> LaunchResolution:
    ...
    its internal ACES source; a routed internal source id is not a direct launch
    choice; an unrouted registered ACES source launches directly; everything else
    is legacy. Launchability reflects the fail-closed catalog entry (an
    unresolved/non-conformant ACES route is not launchable).
    """
    ...
    if source_id is not None:
        ...
        return LaunchResolution(
            scenario_id, source_id, is_aces=True, launchable=bool(entry and entry.get("launchable"))
        )
    if scenario_id in routed_source_ids():
        return LaunchResolution(scenario_id, None, is_aces=True, launchable=False)
    entry = get_catalog_entry(scenario_id)
    if entry is not None and entry.get("scenario_type") == "aces":
        return LaunchResolution(scenario_id, scenario_id, is_aces=True, launchable=bool(entry.get("launchable")))
    launchable = bool(entry.get("launchable", True)) if entry is not None else False
    return LaunchResolution(scenario_id, None, is_aces=False, launchable=launchable)
