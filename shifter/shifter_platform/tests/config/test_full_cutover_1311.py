...
def test_shipped_raes_packs_are_digest_bound_and_valid():
    packs = load_inbox_manifest(SHIPPED_INBOX_MANIFEST)
    # The shipped in-box seed is smoke-linux plus the polaris/NORTHSTORM launch
    # pack (the RAES-native docker-host + prepromoted-DC scenario).
    assert {p.scenario_id for p in packs} == {"smoke-linux", "polaris"}

    for source in packs:
        assert source.contract_kind == "raes"
        assert source.contract_profile == "shifter"
        # The pack's package_ref is relative to RAES_PACKAGE_ROOT, which is the
        ...
        assert validate_pack(pack_root) == source.scenario_id
