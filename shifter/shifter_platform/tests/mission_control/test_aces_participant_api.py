"""End-to-end tests for the Mission Control ACES participant-runtime read APIs (#1288).

Drives the real DRF endpoints with ``APIClient`` against real ``RangeInstance``
/ ``Request`` rows and seeded ``AcesParticipantRuntimeRecord`` sidecar rows.
...
``tests/mission_control/test_aces_api.py`` for operation records.
"""
...
def _implementations_url(request_id) -> str:
    return f"/api/v1/mission-control/range/{request_id}/aces/participant-implementations/"


def _runtimes_url(request_id) -> str:
    return f"/api/v1/mission-control/range/{request_id}/aces/participant-runtimes/"


@pytest.fixture
def user(django_user_model):
    return django_user_model.objects.create_user(username="aces-pr-mc@example.com", email="aces-pr-mc@example.com")


@pytest.fixture
def other_user(django_user_model):
    return django_user_model.objects.create_user(
        username="aces-pr-other@example.com", email="aces-pr-other@example.com"
    )
...
def _token(user, *granted_scopes: str) -> str:
    _, raw = ApiToken.create_token(name="aces-pr-mc", created_by=user, scopes=list(granted_scopes))
    return raw
