...
bind pipeline is exercised without standing up a full canonical ACES pack). The
...
def test_source_backed_file_and_directory_produce_bindings(tmp_path: Path):
    ...
    assert flag.storage_key == f"aces/content/{flag.sha256[:2]}/{flag.sha256}"
...
def test_promotion_is_idempotent(tmp_path: Path):
    ...
    existing = {f"aces/content/{flag_digest[:2]}/{flag_digest}"}
