...
Drives ``shared.aces.operation_input``: the bounded, byte-free projection the
Engine materializes into ``OperationInput`` and the provisioner consumes by
canonical ``operation_id`` instead of reading ``engine_aces_content_delivery_binding``,
``engine_aces_image_mapping``, ``mission_control_range``, and ``engine_instance``.
...
from shared.aces.operation_input import (
...
def _plan() -> dict:
    """A serialized ACES ProvisioningPlan with one sourced and one source-less node."""
    return {
...
def _built(**overrides: object) -> dict:
    ...
    return build_aces_operation_input(**kwargs)  # type: ignore[arg-type]
...
class TestImageLookupKey:
    ...
    def test_malformed_plan_fails_closed(self):
        with pytest.raises(AcesOperationInputError):
...
class TestRoundTrip:
    def test_parse_returns_the_built_projection(self):
        parsed = parse_aces_operation_input(_built())
    ...
    def test_unreferenced_key_resolves_to_no_candidates_not_an_error(self):
        ...
        assert parse_aces_operation_input(_built()).image_candidates_for("gce", "windows") == []

    def test_absent_backend_is_preserved_as_none(self):
        parsed = parse_aces_operation_input(_built(range_backend=None))
...
class TestFailsClosed:
    ...
    def test_non_mapping_payload_is_rejected(self):
        with pytest.raises(AcesOperationInputError):
            parse_aces_operation_input(["not", "a", "mapping"])
