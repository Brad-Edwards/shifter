"""Behavior tests for the object-storage-backed RAES pack staging resolver (#1567).
...
from shared.raes.object_source import _reject_unsafe_member, stage_object_pack
...
class TestStageObjectPackFailsClosed:
    def test_unconfigured_bucket_or_key(self):
        ...
        with pytest.raises(RaesPackageError), _stage(storage, bucket=""):
            pass
        with pytest.raises(RaesPackageError), _stage(storage, key=""):
            pass
    ...
    def test_head_size_over_cap_fails_before_download(self):
        ...
        with pytest.raises(RaesPackageError), _stage(storage):
            pass
    ...
    def test_too_many_entries(self):
        ...
        with pytest.raises(RaesPackageError), _stage(storage, max_entries=3):
            pass

    def test_uncompressed_size_over_cap(self):
        storage = _FakeStorage(_tar([("mypack/big.bin", b"x" * 4096)]))
        with pytest.raises(RaesPackageError), _stage(storage, max_uncompressed_bytes=1024):
            pass
    ...
    def test_storage_failure_maps_to_raes_error(self):
