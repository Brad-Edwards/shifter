"""Unit tests for the RAES source-backed content delivery contract (#1564, S1).
...
def test_binding_transport_roundtrip():
    binding = DeliveryBinding(
        ...
        sha256=_DIGEST,
        storage_key="raes-content/aa/" + _DIGEST,
        byte_count=12,
    )
...
def test_normalized_storage_key_is_content_addressed():
    key = normalized_storage_key("raes-content", _DIGEST)
    assert key == f"raes-content/{_DIGEST[:2]}/{_DIGEST}"


def test_normalized_storage_key_normalizes_prefix():
    assert normalized_storage_key("raes-content/", _DIGEST) == normalized_storage_key("raes-content", _DIGEST)


def test_normalized_storage_key_rejects_bad_digest():
    with pytest.raises(ContentDeliveryError):
        normalized_storage_key("raes-content", "nothex")
