"""Battery for the RAES-native RuntimeTarget provisioning backend (ADR-031, ADR-032).

Covers the interpret step (validate a compiled ProvisioningPlan, then serialize it
verbatim) on both hand-built plans and a plan compiled by the real raes
...
from shared.raes.runtime_target import (
...
def test_interpret_serializes_full_plan_verbatim() -> None:
    ...
    assert serialized["kind"] == RAES_PROVISIONING_PLAN_KIND
    assert serialized["contract_version"] == RAES_PROVISIONING_PLAN_CONTRACT_VERSION  # ADR-032-R7 transport version
    assert serialized["raes_version"]  # stamped from the installed raes
...
def test_raes_plan_contract_rejects_mixed_runtime_domains() -> None:
...
def test_raes_plan_contract_rejects_unknown_provisioning_resource_type() -> None:
...
def test_retained_account_features_pass_declaration_and_evidence(spec: dict) -> None:
    # Every retained feature must clear both the manifest declaration and the
    # independent public RAES realization envelope (#1563).
    plan = _plan(
        _node("provision.node.a", "a"),
        _account_placement("provision.account.a", target="provision.node.a", **spec),
    )
...
def test_evidence_policy_is_independent_of_manifest_declaration() -> None:
    # Widening the manifest must not widen the public realizer configuration.
    plan = _plan(
        _node("provision.node.a", "a"),
    ...
    # ... but the realizer-configuration gate fails closed.
    assert any(
...
def test_declared_but_unrealized_account_feature_fails_validate_and_apply(monkeypatch: pytest.MonkeyPatch) -> None:
    # The same realizer-configuration gate serves validate() and apply() on the
    # one pure path, and apply() never dispatches an unrealized-feature plan.
    monkeypatch.setattr("shared.raes.runtime_target.SHIFTER_PROVISIONER_CAPABILITIES", _mail_overclaimed_capabilities())
...
def test_operation_only_account_overclaim_fails_and_does_not_dispatch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # cannot bypass the realization envelope.
    monkeypatch.setattr("shared.raes.runtime_target.SHIFTER_PROVISIONER_CAPABILITIES", _mail_overclaimed_capabilities())
    port = FakeDispatchPort()
    plan = _plan_ops(
        [_node("provision.node.a", "a")],
