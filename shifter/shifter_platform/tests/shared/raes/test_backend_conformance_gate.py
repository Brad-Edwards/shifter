"""Issue #1263: RAES backend conformance gate.

This is the automated conformance gate for Shifter's initial RAES backend
profile claim. Shifter publishes a single ``provisioning-only`` backend manifest
(``shared.raes.manifest`` + the checked-in
``shifter/shifter_platform/shared/raes/backend-manifest.json`` artifact, issue
#1261). This gate proves that claim stays honest by validating it through
RAES-owned contract/profile/conformance tooling -- Shifter does not reimplement
profile inference, required-contract calculation, or the conformance verdict
(see ``docs/adr/index.yaml`` and
...
1. it runs the RAES fixture conformance runner for the ``provisioning-only``
...
def run_backend_conformance_gate(
    ...
    *,
    ...
    """Validate ``manifest`` against ``expected_profile`` via RAES-owned tooling.

    Returns ``(passed, diagnostics)`` where ``diagnostics`` is a tuple of
    bounded, sanitized single-line strings. RAES tooling is the authority:
    ...
    to the canonical RAES corpus); tests use it to exercise the profile-load
    failure path.
    """
    ...
    if not report.passed:
        diagnostics.append(
            _sanitize_diagnostic(f"{manifest_ref}: RAES fixture conformance failed for profile {expected_profile_id!r}")
        )
...
def test_shifter_backend_manifest_conformance_gate_passes():
    ...
    passed, diagnostics = run_backend_conformance_gate(
        ...
        manifest_ref="shared.raes.manifest",
    )
...
def test_provisioning_only_fixture_suite_is_non_vacuous_and_passes():
    """The RAES fixture runner the gate relies on actually validates cases."""
...
def test_checked_in_artifact_validates_as_provisioning_only_through_raes_tooling():
    """The published ``backend-manifest.json`` artifact validates through the RAES
...
def test_live_target_conformance_provisioning_only_is_non_vacuous():
    """The Shifter RuntimeTarget backend passes the RAES *live* target probe.

    ``run_target_conformance`` drives the backend through the RAES
