"""Admission tests for RAES 2.0.0 authored identity-domain topology (#1606)."""
...
from shared.raes.runtime_target import ShifterProvisioner, interpret_provisioning_plan
...
def test_real_compiled_domain_topology_is_admitted_and_dispatched() -> None:
    ...
    record = RaesOperationRecordData(
        request_id="11111111-1111-1111-1111-111111111111",
        operation_id="domain-topology-probe",
        idempotency_key="domain-topology-probe",
        record_kind="runtime_snapshot",
        contract_kind="raes",
        contract_version="runtime-snapshot-v1",
        contract_profile="provisioning-only",
        source_timestamp=datetime(2026, 7, 28, tzinfo=UTC),
        payload_digest=canonical_raes_payload_digest(payload),
        payload=payload,
    )
    assert validate_raes_operation_record(record).payload == payload
...
def test_domain_topology_diagnostic_messages_are_replaced_without_changing_code(
    ...
    monkeypatch.setattr(
        "shared.raes.domain_topology.domain_topology_plan_diagnostics",
