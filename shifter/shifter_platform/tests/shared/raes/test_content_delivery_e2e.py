...
source-backed ``directory`` in a real RAES SDL, compiles it through the upstream
...
(``engine/provisioner/tests/test_raes_content_delivery*``), which meet this half
at the ``DeliveryBinding`` transport contract. The full live-guest proof is the
``run_raes_backend_validation`` cutover gate (docs/adr/index.yaml).
"""
...
from shared.raes.content_delivery import DeliveryProjection, DeliveryProjectionEntry, sha256_hex
from shared.raes.content_delivery_prep import DeliveryTarget, InventoryEntry, prepare_content_delivery
...
def test_source_backed_content_admitted_and_bound_end_to_end(tmp_path: Path):
    # Compile the authored source-backed content through the real RAES compiler.
    ...
    for binding in bindings:
        assert binding.storage_key == f"raes/content/{binding.sha256[:2]}/{binding.sha256}"
