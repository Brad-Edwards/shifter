...
read surface (``shared.AuditLog``, rehomed from ``risk_register.AuditLog`` in
#1374) and on the boolean success signal. The one failure-path test triggers a
real serialization rejection (a non-JSON payload) rather than mocking the
writer to raise, exercising the "audit logging never breaks the caller" swallow
through a real boundary fault.
