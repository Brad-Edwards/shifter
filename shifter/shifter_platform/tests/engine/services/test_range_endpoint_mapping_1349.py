...
def _gcp_instance(uuid: str, os_type: str, ip: str, username: str, *, rdp_secret: str | None = None) -> dict:
...
class TestLinuxOnlyComposition:
    ...
    def test_ssh_resolves_for_every_linux_member(self, settings, user):
        ...
        _range(
            user,
            [
                _gcp_instance("comp-a-kali", "kali", "10.50.1.10", "kali"),
                _gcp_instance("comp-a-ubuntu", "ubuntu", "10.50.1.11", "ubuntu"),
            ],
        )
    ...
    def test_rdp_not_offered_for_linux_members(self, settings, user):
        # A Linux member carries no per-instance RDP password secret (the
        # provisioner mints one only for GUI/Windows guests), so RDP is refused
        # -- the same credential-presence gate the AWS path uses.
        from engine.services import get_rdp_connection_info

        settings.CLOUD_PROVIDER = "aws"
        _range(user, [_gcp_instance("comp-a-ubuntu", "ubuntu", "10.50.1.11", "ubuntu")])
        with pytest.raises(ValueError, match="RDP credentials are not available"):
...
class TestWindowsMixedComposition:
    ...
    def test_ssh_for_kali_and_rdp_for_windows(self, settings, user):
        ...
        _range(
            user,
            [
                _gcp_instance("comp-b-kali", "kali", "10.50.2.10", "kali"),
                _gcp_instance(
                    "comp-b-win",
                    "windows",
                    "10.50.2.20",
...
class TestDestroyRevocation:
    ...
    def test_ssh_refused_when_range_not_ready(self, settings, user, status):
        ...
        settings.CLOUD_PROVIDER = "aws"
        _range(user, [_gcp_instance("revoked-kali", "kali", "10.50.3.10", "kali")], status=status)
