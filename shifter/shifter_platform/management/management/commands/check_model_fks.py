"""Check for cross-layer ForeignKey relationships between models.

This management command inspects Django models across the platform's
service layers and detects FK/OneToOne/M2M relationships that cross
layer boundaries.

## Layer Boundaries

  - shared        - common utilities
  - engine        - infrastructure provisioning
  - cms           - content management
  - management    - platform administration
  - mission_control - presentation layer

## Rules

NO cross-layer FKs allowed. Zero coupling between layers.
Each layer's models can only FK to models within the same layer.

FKs to Django's auth.User and other built-in apps are allowed.

Usage:
    python manage.py check_model_fks
    python manage.py check_model_fks --json
    python manage.py check_model_fks -o report.json
"""

import json
import sys
from typing import Any

from django.apps import apps
from django.core.management.base import BaseCommand

# All service layers. Every first-party Django app is classified (ADR-001,
# #1523); held to set-equality with layer_imports.yaml by tests.
ALL_LAYERS = ["shared", "engine", "cms", "management", "mission_control", "ctf", "config"]

# Apps that are allowed to be referenced from any layer
ALLOWED_EXTERNAL_APPS = {"auth", "contenttypes", "sessions", "admin"}

# Reverse relation types (auto-generated by Django, not actual FK definitions)
REVERSE_RELATION_TYPES = {"ManyToOneRel", "OneToOneRel", "ManyToManyRel"}


def is_violation(from_layer: str, to_layer: str) -> bool:
    """Check if a FK from from_layer to to_layer is a violation.

    ANY cross-layer FK is a violation. Zero coupling between layers.
    """
    if from_layer not in ALL_LAYERS or to_layer not in ALL_LAYERS:
        return False
    return from_layer != to_layer  # Any cross-layer FK is a violation


def get_layer_for_app(app_label: str) -> str | None:
    """Map Django app label to layer name."""
    # Direct mapping for our layers
    if app_label in ALL_LAYERS:
        return app_label
    # mission_control might be registered differently
    if app_label == "mission_control":
        return "mission_control"
    return None


def _analyze_fk_field(layer: str, model: type, field: object) -> dict[str, Any] | None:
    """Return cross-layer FK info for one model field, or None if it is not a tracked relation."""
    related_model = getattr(field, "related_model", None)
    # Skip reverse relations, non-relation fields, and self-referential string relations.
    if type(field).__name__ in REVERSE_RELATION_TYPES or not related_model or isinstance(related_model, str):
        return None

    related_app = related_model._meta.app_label
    related_layer = get_layer_for_app(related_app)
    # Skip allowed external apps and same-layer / unmapped relations.
    if related_app in ALLOWED_EXTERNAL_APPS or not related_layer or related_layer == layer:
        return None

    return {
        "model": model.__name__,
        "field": getattr(field, "name", ""),
        "field_type": type(field).__name__,
        "references": f"{related_layer}.{related_model.__name__}",
        "to_layer": related_layer,
        "is_violation": is_violation(layer, related_layer),
    }


class Command(BaseCommand):
    help = "Check for cross-layer ForeignKey violations between models"

    def add_arguments(self, parser):
        parser.add_argument(
            "--json",
            action="store_true",
            help="Output results as JSON",
        )
        parser.add_argument(
            "-o",
            "--output",
            metavar="FILE",
            help="Save JSON output to file",
        )
        parser.add_argument(
            "-q",
            "--quiet",
            action="store_true",
            help="Suppress summary output",
        )

    def handle(self, *args, **options):
        results = self.analyze_fks()
        stats = self.compute_stats(results)

        output = {
            "relationships": results,
            "stats": stats,
        }

        # Handle output
        if options["output"]:
            with open(options["output"], "w") as f:
                json.dump(output, f, indent=2)
            self.stderr.write(f"Output saved to {options['output']}")

        if options["json"] and not options["output"]:
            self.stdout.write(json.dumps(output, indent=2))

        # Print summary unless quiet
        if not options["quiet"]:
            self.print_summary(results, stats)

        # Exit with error if violations found
        if stats["violations"] > 0:
            sys.exit(1)

    def analyze_fks(self) -> dict[str, list[dict[str, Any]]]:
        """Analyze all FK relationships across layers."""
        results: dict[str, list[dict[str, Any]]] = {layer: [] for layer in ALL_LAYERS}

        for layer in ALL_LAYERS:
            try:
                app_config = apps.get_app_config(layer)
            except LookupError:
                continue

            for model in app_config.get_models():
                for field in model._meta.get_fields():
                    rel_info = _analyze_fk_field(layer, model, field)
                    if rel_info is not None:
                        results[layer].append(rel_info)

        return results

    def compute_stats(self, results: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
        """Compute summary statistics."""
        stats: dict[str, Any] = {
            "total_cross_layer_fks": 0,
            "violations": 0,
            "clean_layers": [],
            "layers_with_violations": [],
            "violation_details": [],
        }

        for layer, relationships in results.items():
            layer_violations = [r for r in relationships if r["is_violation"]]

            stats["total_cross_layer_fks"] += len(relationships)
            stats["violations"] += len(layer_violations)

            if layer_violations:
                stats["layers_with_violations"].append(layer)
                for v in layer_violations:
                    stats["violation_details"].append(
                        {
                            "from": layer,
                            "model": v["model"],
                            "field": v["field"],
                            "references": v["references"],
                        }
                    )
            else:
                stats["clean_layers"].append(layer)

        return stats

    def print_summary(self, results: dict, stats: dict):
        """Print human-readable summary."""
        self.stdout.write("\n" + "=" * 50)
        self.stdout.write("MODEL FK SUMMARY")
        self.stdout.write("=" * 50)

        self.stdout.write(f"\nTotal cross-layer FKs: {stats['total_cross_layer_fks']}")
        self.stdout.write(f"Violations: {stats['violations']}")

        self.stdout.write(f"\nClean layers: {', '.join(stats['clean_layers']) or 'none'}")

        if stats["violation_details"]:
            self.stdout.write("\nViolations detected:")
            for v in stats["violation_details"]:
                self.stdout.write(f"  {v['from']}.{v['model']}.{v['field']} -> {v['references']}")
        else:
            self.stdout.write(self.style.SUCCESS("\nNo violations detected!"))

        self.stdout.write("=" * 50 + "\n")
