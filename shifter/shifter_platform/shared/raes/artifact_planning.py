...
def plan_with_artifact_supply(scenario, target, *, parameters=None, supply_provider=None):
    ...
    for item in compiled.realization_requirements:
        if item.artifact_requirement is not None:
    ...
    if isinstance(supplied, ArtifactSupply):
        ...
        supply = ArtifactSupply(supplied, (), ())
    else:
        supply = build_artifact_supply(requirements, [])
    ...
    for address, requirement in requirements.items():
        ...
        if result.status is ArtifactResolutionStatus.SATISFIED:
