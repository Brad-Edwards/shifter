"""Capability-envelope validation for RAES composition placements (ADR-032).
...
Account features are gated against two independent public RAES contracts (#1563):
the manifest declaration (``supported_account_features``, plan-time vocabulary)
and the configuration-bound RAES ``RealizerConfigurationModel``. A requested
feature outside the declaration is an ``unsupported-account-feature``; a feature
that is declared but absent from the realizer configuration is an
``account-feature-not-realized`` -- so the manifest cannot silently over-claim
account realization.
"""
...
from raes_contracts.planning import ChangeAction, PlannedResource, PlanOperation

from shared.log_sanitize import safe_log_value
from shared.raes.realization import REALIZED_ACCOUNT_FEATURES
...
_REALIZED_FEATURE_TYPES: frozenset[str] = frozenset({"service", "artifact", "configuration"})
...
def _account_feature_diagnostics(
    ...
    Two independent RAES contracts (#1563): the manifest declaration
    (``supported_account_features``) and the realizer configuration's material
    configuration (``REALIZED_ACCOUNT_FEATURES``). A requested feature outside the
    declaration is ``unsupported-account-feature``; a declared feature absent from
    the realization envelope is ``account-feature-not-realized``. The branches are
    mutually exclusive so a single feature never double-reports.
    """
    diagnostics = [
...
def _feature_shape_diagnostics(address: str, payload: Mapping[str, object]) -> list[Diagnostic]:
    ...
    diagnostics: list[Diagnostic] = []
    if feature_type not in _REALIZED_FEATURE_TYPES:
        return [
            _diagnostic(
