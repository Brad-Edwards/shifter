"""Realizability-model representation of parameterized ACES scenario runs (#1579).
...
This module is that representation. It reuses the ACES SDL parameterization
contract (``aces-sdl``): declared ``variables`` are the parameter schema and
...
Like the other ``shared.aces`` modules it may import ``aces-sdl`` (ADR-031-R1);
callers outside ``shared.aces`` (for example ``cms.scenarios``) use this seam
...
from aces_sdl.instantiate import SDLInstantiationError, instantiate_scenario
...
_MAX_DIAGNOSTICS = 20
#: Parse/read failures aces-sdl may raise for a malformed or unreadable document.
...
class RunRepresentationError(Exception):
    ...
    never raw ACES error text, which may echo SDL fragments or input values.
    """


@dataclass(frozen=True)
class RunParameter:
    """Bounded declaration projection of one ACES SDL ``variable``.
...
def validate_run_binding(
    ...
    *,
    ...
    """Validate a proposed parameter binding against declared ACES variables.

    Delegates the authoritative check to ``aces_sdl.instantiate_scenario`` (which
