"""Read-only projections of ACES participant-runtime sidecar records (#1288).

Mirrors :mod:`shared.aces.projections` (the incumbent
:class:`shared.models.AcesOperationRecord` read seam from #1275). The sidecar
(:class:`shared.models.AcesParticipantRuntimeRecord`) is written through
:mod:`shared.aces.participant_runtime` after
:mod:`shared.schemas.aces_participant_runtime` validation, which already
...
# ``shared.schemas.aces_participant_runtime.PAYLOAD_KEYS_BY_RECORD_KIND``;
...
RESPONSE_PAYLOAD_KEYS_BY_RECORD_KIND: dict[str, frozenset[str]] = {
    AcesParticipantRuntimeRecord.RecordKind.PARTICIPANT_IMPLEMENTATION: frozenset(
        {
            "backend_name",
            "capability_refs",
    ...
    AcesParticipantRuntimeRecord.RecordKind.PARTICIPANT_RUNTIME: frozenset(
        {
            "participant_ref",
            "runtime_digest",
            "runtime_ref",
...
class AcesParticipantRuntimeRecordProjection:
    """Serializer-ready, redacted view of one :class:`AcesParticipantRuntimeRecord`.
...
def _redacted_payload(record: AcesParticipantRuntimeRecord) -> dict[str, Any]:
...
def _project(record: AcesParticipantRuntimeRecord) -> AcesParticipantRuntimeRecordProjection:
    ...
    return AcesParticipantRuntimeRecordProjection(
        id=record.id,
...
def list_participant_runtime_records(
    ...
    *,
...
) -> list[AcesParticipantRuntimeRecordProjection]:
    ...
    ``AcesParticipantRuntimeRecord`` model. The per-kind response allowlist is
    ...
    rows = AcesParticipantRuntimeRecord.objects.filter(**filters).order_by("-source_timestamp", "-created_at")[:bounded]
