...
class WorkspaceOwnershipTransferResult:
    ...
    workspace_uuid: UUID
    outcome: str  # "transferred" | "blocked_no_membership"
...
def admin_transfer_workspace_ownership(
    *,
    ...
    responsibility (staff session + ``auth.change_user``); this service is never
    reachable through workspace-role authority and must not be called from a
    tenant-facing path.
    ...
    with transaction.atomic():
        ...
        for source_membership in owned:
            workspace = Workspace.objects.select_for_update().get(pk=source_membership.workspace_id)
            target = (
                WorkspaceMembership.objects.select_for_update()
                .filter(workspace=workspace, user_id=new_owner_user_id)
                .first()
            ...
            if target is None:
                results.append(WorkspaceOwnershipTransferResult(workspace.uuid, "blocked_no_membership"))
                continue

            # Complete the offboarding even when the replacement already owns the
            # workspace: the departing source is still an owner (this loop only
            # visits its OWNER memberships), so it must be demoted so it no longer
            # owns the workspace (issue #1943 review cycle-2 F1, ADR-046-R13).
            if target.role != WorkspaceRole.OWNER.value:
                target.role = WorkspaceRole.OWNER.value
                target.save(update_fields=["role", "updated_at"])
            ...
            results.append(WorkspaceOwnershipTransferResult(workspace.uuid, "transferred"))
    return results
