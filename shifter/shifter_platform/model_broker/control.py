...
class ControlClient:
    ...
    def __init__(self, *, url: str, ca_file: str, identity):
        parts = urlsplit(url)
        if (
            parts.scheme != "https"
    ...
    async def call(self, route: str, payload: dict) -> dict:
        if route not in {"authenticate", "exchange", "refresh", "reserve", "advance", "finish"}:
            ...
            async with asyncio.timeout(2):
                ...
                async with self.client.stream(
                    ...
                    if response.status_code != 200:
                        # Only Engine's fixed vocabulary crosses this boundary.
                        code = data.get("error", "")
                        if code not in {
