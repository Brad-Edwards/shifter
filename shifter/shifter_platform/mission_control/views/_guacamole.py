...
def guacamole_rdp_url(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        # ``conn_info`` carries RDP credentials (password / ssh_key). Never log
        # any value read from it: only derive non-secret metadata (os type, and
        # whether a key is present as a yes/no flag — not the key itself) and
        # route it through ``safe_log_value`` so the secret-bearing source is
        # not surfaced and CodeQL's clear-text-logging dataflow is broken.
...
def guacamole_ssh_url(request: HttpRequest) -> JsonResponse:
    ...
    # ``ssh_info`` carries the SSH private key. Never log any value read from
    # it directly: derive only non-secret metadata (host IP, cloud provider
    # name) and route it through ``safe_log_value`` so the secret-bearing
    # source is not surfaced and CodeQL's clear-text-logging dataflow is broken.
    instance_ip = str(ssh_info["host"])
    cloud_provider_name = str(ssh_info.get("cloud_provider") or "unknown")
    logger.info(
        "Guacamole SSH bootstrap queued for range instance: user=%s instance_uuid=%s host=%s provider=%s",
        safe_log_value(user.email),
        safe_log_value(instance_uuid),
        safe_log_value(instance_ip),
        safe_log_value(cloud_provider_name),
    )
    return guacamole_bootstrap_response(
        user=user,
