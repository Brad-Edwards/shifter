...
The pure half -- payload construction, HMAC signing/encryption, and the
per-protocol connection parameter maps -- lives in
:mod:`mission_control._guacamole_payload` (Sonar S104) and is re-exported here,
so this module's public surface is unchanged. What remains is the network-facing
half: the ``/api/tokens`` exchange with its bounded readiness retry, and the
signed-URL builders callers use.
...
from mission_control._guacamole_payload import (
    RDPConnectionParams,
