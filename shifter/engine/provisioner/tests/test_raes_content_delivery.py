"""Tests for the #1564 post-boot RAES content-delivery orchestration.

Covers the two halves of ``raes_content_delivery``:
...
- ``assert_content_delivery_bindings_complete``: the fail-closed gate joining
  ...
  binding at all).
- ``realize_raes_content_delivery``: the download+digest-verify+guest-delivery
  ...
  ``RaesContentDeliveryPlan`` scripts/context flow through, and the explicit
...
def _node(address: str = "node.web", os_family: str = "linux", count: int = 1) -> RaesPlanNode:
    return RaesPlanNode(
        address=address, name=address.rsplit(".", 1)[-1], os_family=os_family, count=count, network_addresses=()
    )


def _feature(**kw) -> RaesPlanFeature:
    base = {
        "name": "agent",
...
def _binding(**kw) -> dict[str, Any]:
    ...
    ``shared.raes.content_delivery.normalized_storage_key`` so a test that
    ...
    base = {
        ...
        "sha256": digest,
        "storage_key": "raes/content-delivery/" + digest[:2] + "/" + digest,
        "byte_count": 5,
        "binding_version": 1,
    }
    base.update(kw)
...
class TestAssertContentDeliveryBindingsComplete:
    ...
    def test_duplicate_content_address_fails_closed(self):
        ...
        with pytest.raises(RaesGceCompositionError, match="duplicate resource identity"):
    ...
    def test_binding_that_fails_contract_validation_fails_closed(self):
        ...
        with pytest.raises(RaesGceCompositionError, match="failed contract validation"):
            assert_content_delivery_bindings_complete(plan, [binding])
    ...
    def test_feature_environment_fails_before_cloud_realization(self):
        ...
        binding = _feature_binding()
        with pytest.raises(RaesGceCompositionError, match="no safe realization contract"):
            assert_content_delivery_bindings_complete(plan, [binding])
...
def _ops(
    *,
    ...
    def execution_builder(output, **_kwargs):
        default_results = [_success("RAES_CONTENT_FILE_INSTALLED"), _success("RAES_CONTENT_FILE_VERIFIED")]
...
class TestRealizeRaesContentDelivery:
    def test_no_source_backed_content_is_a_noop(self):
        plan = _plan(content=(_content(source_name=None, text="hello"),))
        ops, storage, _ = _ops()
        realize_raes_content_delivery(raes_plan=plan, instance_outputs=[], delivery_bindings=[], ops=ops)
    ...
    def test_feature_artifact_is_installed_executable_and_verified(self):
        ...
        assert len(storage.download_calls) == 1
        assert "/opt/raes/agent" in executors[0].calls[0]["script"]
    ...
    def test_feature_realization_preserves_cross_shape_dependency_order(self):
        ...
        assert "nginx" in executors[0].calls[0]["script"]
        assert "/opt/raes/agent" in executors[1].calls[0]["script"]
    ...
    def test_feature_dependency_cycle_fails_before_guest_realization(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="dependencies contain a cycle"):
            realize_raes_content_delivery(
                raes_plan=plan,
    ...
    def test_provisioner_side_digest_mismatch_fails_before_any_guest_call(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="digest mismatch"):
            realize_raes_content_delivery(
                raes_plan=plan,
    ...
    def test_unconfigured_bucket_fails_closed(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="bucket is not configured"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
    ...
    def test_download_failure_is_wrapped_value_free(self, caplog):
        ...
        with pytest.raises(RaesContentDeliveryError) as exc_info:
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
    ...
    def test_in_guest_verify_step_failure_fails_closed_before_ready(self, monkeypatch):
        ...
        realize_raes_content_delivery must preserve the RAES-specific error so
        ...
        executor = _FakeExecutor(
            results=[_success("RAES_CONTENT_FILE_INSTALLED"), _failure("FATAL: readback digest mismatch")]
        )
        ops, _storage, executors = _ops(executor=executor)
        output = _output("node.web#0")
        binding = _binding(byte_count=5)

        with pytest.raises(RaesContentDeliveryError, match="in-guest digest verification failed"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
    ...
    def test_feature_service_verify_step_failure_reports_verification_error(self, monkeypatch):
        ...
        executor = _FakeExecutor(
            results=[_success("RAES_FEATURE_SERVICE_INSTALLED"), _failure("FATAL: service not active")]
        )
        ops, _storage, executors = _ops(executor=executor)
        instance_outputs = [_output("node.web#0")]

        with pytest.raises(RaesContentDeliveryError, match="feature service verification failed"):
            realize_raes_content_delivery(
                raes_plan=plan,
    ...
    def test_deliver_step_failure_fails_closed(self, monkeypatch):
        ...
        with pytest.raises(RaesContentDeliveryError, match="setup plan failed"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
    ...
    def test_guest_not_ready_fails_closed(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="did not become ready"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
    ...
    def test_missing_instance_output_fails_closed(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="instance output is missing"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
...
def _storage_download_count(ops: RaesContentDeliveryOps) -> int:
...
class TestBindingContractValidation:
    ...
    def test_unsupported_binding_version_fails_closed_before_any_guest_call(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="binding is invalid"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
    ...
    def test_downloaded_size_disagreeing_with_byte_count_fails_closed(self):
        ...
        with pytest.raises(RaesContentDeliveryError, match="size mismatch"):
            realize_raes_content_delivery(
                raes_plan=plan,
                instance_outputs=[output],
                delivery_bindings=[binding],
...
class TestDirectoryContentInstalledTreeDigest:
    ...
    def test_directory_content_computes_and_delivers_installed_tree_digest(self):
        ...
        # real RaesContentDeliveryPlan/_verify_stdin), never the tar digest,
