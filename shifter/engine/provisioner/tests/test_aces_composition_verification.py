"""Tests for synchronous in-guest ACES composition verification (#1569)."""
...
def _node(*, os_family: str = "linux", count: int = 2) -> AcesPlanNode:
    return AcesPlanNode(
        address="node.web",
        name="web",
        os_family=os_family,
...
def _plan(*, node: AcesPlanNode | None = None) -> AcesPlan:
...
def _windows_plan() -> AcesPlan:
    plan = _plan(node=_node(os_family="windows", count=1))
    return replace(
        plan,
        content=(
            replace(plan.content[0], path="C:\\aces\\value.txt"),
            replace(plan.content[1], destination="C:\\aces\\empty"),
        ),
    )


def _account_variant_plan(*, os_family: str) -> AcesPlan:
...
def test_verifies_every_bootstrap_item_on_every_concrete_instance() -> None:
    ...
    assert "ACES_COMPOSITION_VERIFIED" in rendered
...
def test_windows_shell_or_home_is_rejected_before_mutation() -> None:
    ...
    with pytest.raises(AcesGceCompositionError, match="unsupported account attributes"):
...
def test_source_less_file_without_inline_bytes_is_not_overclaimed() -> None:
    ...
    with pytest.raises(AcesGceCompositionError, match="unsupported content shape"):
        assert_composition_is_verifiable(plan)
...
def test_windows_plan_checks_real_file_directory_account_and_acl_state() -> None:
    ...
    assert "C:\\aces\\value.txt" not in verify_step.script
...
def test_windows_probe_crosses_the_static_script_and_stdin_executor_boundary() -> None:
    ...
    executor.run_command.return_value = CommandResult(
        success=True,
        exit_code=0,
        stdout="ACES_COMPOSITION_VERIFIED",
        stderr="",
    )
    execution = _Execution()
    ...
    assert "C:\\aces\\value.txt" not in command.kwargs["script"]
