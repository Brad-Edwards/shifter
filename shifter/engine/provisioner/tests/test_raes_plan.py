"""Tests for the provisioner-side serialized-RAES-plan reader (ADR-031, ADR-032).

The reader consumes the serialized RAES ProvisioningPlan persisted in
range_config and extracts realization intent via accessors mirroring the
reference RAES backend. These tests drive it directly with serialized-plan dicts;
a platform-side drift test compares its extraction against raes_backend_libvirt.
"""
...
class TestCompositionExtraction:
    ...
    def test_rejects_unsupported_account_auth_method(self, auth_method: str):
        ...
        with pytest.raises(RaesPlanError, match="unsupported account auth_method"):
            parse_plan(serialized)
    ...
    def test_rejects_malformed_explicit_account_auth_method(self, auth_method: object):
        ...
        serialized = _serialized(account_resource, self._target_node())
        with pytest.raises(RaesPlanError, match="auth_method must be a canonical string"):
            parse_plan(serialized)
    ...
    def test_rejects_unsafe_password_strength(self, password_strength: str):
        ...
        serialized = _serialized(account_resource, self._target_node())
        with pytest.raises(RaesPlanError, match="unsupported password_strength"):
            parse_plan(serialized)
    ...
    def test_rejects_malformed_explicit_password_strength(self, password_strength: object):
        ...
        serialized = _serialized(account_resource, self._target_node())
        with pytest.raises(RaesPlanError, match="password_strength must be a canonical string"):
            parse_plan(serialized)
    ...
    def test_rejects_username_unsafe_for_supported_guest_dialects(self, username: str):
        ...
        serialized = _serialized(account_resource, self._target_node())
        with pytest.raises(RaesPlanError, match="account username is not portable"):
            parse_plan(serialized)

    @pytest.mark.parametrize("username", ["raes", "RAES"])
    def test_rejects_provisioner_management_username(self, username: str):
        ...
        serialized = _serialized(account_resource, self._target_node())
        with pytest.raises(RaesPlanError, match="reserved for provisioner management"):
            parse_plan(serialized)
    ...
    def test_rejects_mail_in_separate_provisioner_consumer(self):
        ...
        serialized = _serialized(account_resource, self._target_node())
        with pytest.raises(RaesPlanError, match="account mail is not realized"):
            parse_plan(serialized)
    ...
    def test_rejects_spn_without_domain_binding_without_leaking_value(self):
        ...
        with pytest.raises(RaesPlanError, match="account spn requires a supported domain binding") as error:
            parse_plan(serialized)
    ...
    def test_malformed_composition_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match="malformed content-placement"):
            parse_plan(serialized)
...
class TestVersionValidation:
    ...
    def test_missing_contract_version_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match="contract_version"):
            parse_plan(serialized)

    def test_unknown_contract_version_fails_closed(self):
        serialized = _serialized(self._node(), contract_version="raes-provisioning-plan-v99")
        with pytest.raises(RaesPlanError, match="contract_version"):
            parse_plan(serialized)

    def test_missing_raes_version_fails_closed(self):
        serialized = _serialized(self._node(), version=None)
        with pytest.raises(RaesPlanError, match="raes_version"):
            parse_plan(serialized)

    def test_empty_raes_version_fails_closed(self):
    ...
    @pytest.mark.parametrize(
        "version",
        ["0.18.9", "0.19.9", "0.20.0", "0.21.0", "0.22.0", "0.23.0", "1.4.0", "2.0.1", "3.0.0"],
    )
    def test_any_other_raes_release_fails_closed(self, version: str):
        serialized = _serialized(self._node(), version=version)
        with pytest.raises(RaesPlanError, match="unsupported raes_version"):
            parse_plan(serialized)
    ...
    def test_unparseable_raes_version_fails_closed(self):
        serialized = _serialized(self._node(), version="not-a-version")
        with pytest.raises(RaesPlanError, match="not a valid release version"):
            parse_plan(serialized)

    def test_exact_supported_raes_release_accepted(self):
        parsed = parse_plan(_serialized(self._node(), version=SUPPORTED_RAES_VERSION))
        assert parsed.raes_version == SUPPORTED_RAES_VERSION
...
class TestFailClosed:
    ...
    def test_unknown_resource_type_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match="resource_type"):
            parse_plan(serialized)
    ...
    def test_duplicate_network_alias_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match="duplicate network alias"):
            parse_plan(serialized)
    ...
    def test_duplicate_node_alias_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match="duplicate node alias"):
            parse_plan(serialized)
    ...
    def test_dangling_acl_endpoint_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match=r"ACL .* references unknown network"):
            parse_plan(serialized)
    ...
    def test_dangling_composition_target_fails_closed(self):
        ...
        with pytest.raises(RaesPlanError, match="targets unknown node"):
            parse_plan(serialized)
    ...
    def test_unhashable_contract_version_fails_closed(self):
        ...
        # RaesPlanError, not raise TypeError from set membership.
