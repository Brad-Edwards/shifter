...
def _realize_shared_vpc_networks(
    ...
    *,
    ...
    if allocated_network_cidrs is None:
        if not reconstruct_for_teardown:
            raise RaesGceAdapterError("shared-VPC networks require a tenant-allocated subnet projection")
        projection = _teardown_projection(plan.networks)
    else:
        projection = tuple(allocated_network_cidrs)
    ...
    for authored, (_address, cidr) in zip(plan.networks, projection, strict=True):
        ...
        if authored.cidr:
            original = _ipv4_subnet(authored.cidr, source="authored GCE network")
            if candidate.prefixlen != original.prefixlen:
        ...
        realized.append(replace(authored, cidr=str(candidate), gateway=gateway))
...
def _teardown_projection(networks: tuple[RaesPlanNetwork, ...]) -> tuple[tuple[str, str], ...]:
    ...
    if not networks:
        return ()
    authored_prefixes = {
        _ipv4_subnet(network.cidr, source="authored GCE network").prefixlen for network in networks if network.cidr
    }
...
def _rebase_address(value: str, authored_cidr: str | None, realized: ipaddress.IPv4Network) -> str:
    ...
    authored = _ipv4_subnet(authored_cidr, source="authored GCE network")
    try:
