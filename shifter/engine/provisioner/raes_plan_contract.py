...
def prepare_envelope(envelope: Mapping[str, Any], *, cleanup_only: bool) -> Mapping[str, Any]:
    ...
    carrier = envelope.get("realization_envelope")
    if carrier is not None:
        ...
        if (
            ...
            or carrier["schema_version"] != "realization-envelope/v1"
        ):
            raise RaesPlanError("realization_envelope has an unsupported version")
        if any(
            re.fullmatch(r"sha256:[a-f0-9]{64}", carrier[key]) is None for key in ("digest", "configuration_digest")
        ):
...
def _validate_authority(envelope: Mapping[str, Any]) -> None:
    ...
    for row in envelope.get("realization_authority", []):
        ...
        if identity in seen or row["address"] not in envelope.get("resources", {}):
