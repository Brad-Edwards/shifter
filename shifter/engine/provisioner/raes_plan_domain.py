"""Domain/account policy for the serialized RAES plan consumer (ADR-032-R7).
...
def _unsupported_domain_account(account: RaesPlanAccount) -> bool:
...
def validate_account_credentials(account: RaesPlanAccount) -> None:
    ...
    if account.auth_method not in SUPPORTED_ACCOUNT_AUTH_METHODS:
        raise RaesPlanError("unsupported account auth_method")
    _validate_password_credential(account)
    _validate_optional_account_fields(account)


def _validate_password_credential(account: RaesPlanAccount) -> None:
    ...
    if invalid_password or enabled_without_password:
        raise RaesPlanError("unsupported password_strength for account credential")


def _validate_optional_account_fields(account: RaesPlanAccount) -> None:
    """Validate mail, domain binding, policy, and optional SPN fields."""
    if account.mail is not None:
        raise RaesPlanError("account mail is not realized consistently across supported guest operating systems")
    if account.spn is not None and account.domain_ref is None:
        raise RaesPlanError("account spn requires a supported domain binding")
    if account.domain_ref is not None and _unsupported_domain_account(account):
        raise RaesPlanError("domain account policy is unsupported by this provisioner")
    if account.spn is not None and _invalid_spn(account.spn):
        raise RaesPlanError("account spn is invalid for this provisioner")
...
def topology_addresses(topology: Mapping[str, Any], field: str) -> tuple[str, ...]:
    ...
    if not isinstance(value, list | tuple) or malformed_item:
        raise RaesPlanError("domain topology address list is malformed")
...
def topology(payload: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise RaesPlanError("domain topology must be an object")
    result = dict(raw)
...
def _topology_for_node(node: RaesPlanNode) -> Mapping[str, Any]:
...
def _validated_controller(domain_nodes: tuple[RaesPlanNode, ...]) -> RaesPlanNode:
    ...
    if not valid:
        raise RaesPlanError("domain controller cardinality or operating system is unsupported")
    return controllers[0]


def _member_unreachable(controller: RaesPlanNode, member: RaesPlanNode) -> bool:
...
def _validated_members(domain_nodes: tuple[RaesPlanNode, ...], controller: RaesPlanNode) -> tuple[RaesPlanNode, ...]:
    ...
    if any(member.os_family.lower() != "windows" for member in members):
        raise RaesPlanError("domain member operating system is unsupported")
    if any(_member_unreachable(controller, member) for member in members):
        raise RaesPlanError("domain member is not reachable from its controller")
    if any(controller.address not in member.ordering_dependencies for member in members):
        raise RaesPlanError("domain member ordering dependency is missing")
    return members


def _controller_binding(controller: RaesPlanNode) -> tuple[Mapping[str, Any], str, tuple[str, ...]]:
    ...
    if topology_text(value, "profile") != "active_directory" or controller_addresses != (controller.address,):
        raise RaesPlanError("domain topology profile or controller binding is unsupported")
    return value, topology_text(value, "authority_account_address"), controller_addresses


def _valid_authority(
    authority: RaesPlanAccount | None,
    domain_id: str,
...
def _validate_domain_accounts(
    ...
    nodes_by_address: Mapping[str, RaesPlanNode],
) -> None:
...
def _is_unbound_domain_account(account: RaesPlanAccount, domain_id: str, authority_address: str) -> bool:
...
def _bound_domain_accounts(
    ...
    if unbound:
        raise RaesPlanError("domain topology account binding is invalid")
    if any(account.domain_id != domain_id for account in domain_accounts):
        raise RaesPlanError("domain account binding is invalid")
    return domain_accounts


def _validate_unique_domain_identities(
    authority: RaesPlanAccount,
    domain_accounts: tuple[RaesPlanAccount, ...],
) -> None:
    ...
    if len(usernames) != len(set(usernames)):
        raise RaesPlanError("duplicate domain account identity")
    if len(spns) != len(set(spns)):
        raise RaesPlanError("duplicate account spn")
...
def _validate_domain_account_targets(
    ...
    if any(_account_target_outside_domain(account, domain_id, nodes_by_address) for account in domain_accounts):
        raise RaesPlanError("domain account target is invalid")
...
def _build_domain(
    ...
    accounts: tuple[RaesPlanAccount, ...],
    nodes_by_address: Mapping[str, RaesPlanNode],
    accounts_by_address: Mapping[str, RaesPlanAccount],
) -> RaesPlanDomain:
...
def build_domains(nodes: tuple[RaesPlanNode, ...], accounts: tuple[RaesPlanAccount, ...]) -> tuple[RaesPlanDomain, ...]:
    ...
    if any(account.domain_ref is not None and account.domain_ref not in domain_ids for account in accounts):
        raise RaesPlanError("domain account references an unsupported domain")
    return domains
