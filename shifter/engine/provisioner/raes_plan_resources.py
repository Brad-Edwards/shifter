"""Serialized resource collection for the provisioner RAES plan reader."""
...
from raes_plan_domain import topology, topology_signature, topology_text
...
def _record_topology(collection: ResourceCollection, payload: Mapping[str, Any]) -> None:
    ...
    if previous is not None and previous != signature:
        raise RaesPlanError("domain topology identity is inconsistent")
...
def _record_resource(
    ...
    elif resource_type in collection.composition:
        collection.composition[resource_type].append(pair)
    elif resource_type == DOMAIN_CONTROLLER_PLACEMENT_RESOURCE_TYPE:
        collection.dependency_aliases[address] = _string(
            payload.get("target_address"),
            where="domain-controller-placement.target_address",
        )
...
def _collect_resource(collection: ResourceCollection, entry: object) -> None:
    ...
    if address in collection.seen_addresses:
        raise RaesPlanError(f"duplicate resource address {address!r}")
...
def collect_resources(resources: Mapping[str, Any]) -> ResourceCollection:
    ...
    for alias, target in collection.dependency_aliases.items():
        if target not in collection.seen_addresses:
            raise RaesPlanError(f"domain-controller placement {alias!r} targets unknown resource {target!r}")
    for address, dependencies in collection.ordering_dependencies.items():
        collection.ordering_dependencies[address] = tuple(
            dict.fromkeys(collection.dependency_aliases.get(dependency, dependency) for dependency in dependencies)
        )
    return collection
