...
Reduces a parsed serialized ACES plan plus its exact composition proof set to the
...
def snapshot_resources(
    ...
    runtime_snapshot record. Addresses are compiled ACES handles and carry no
    ...
    if verified != expected:
        raise ValueError("ACES composition verification coverage is incomplete")
    resources.extend(
    ...
    if len(encoded) > _MAX_PAYLOAD_BYTES - _PAYLOAD_METADATA_RESERVE_BYTES:
        raise ValueError("ACES runtime snapshot exceeds the persistence size bound")
    return resources
