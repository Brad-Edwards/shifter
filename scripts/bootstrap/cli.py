...
def check_dependencies(command: str | None = None, cloud: str | None = None) -> None:
    ...
    is_gcp_runners = command == "runners" and cloud == Cloud.GCP.value

    aws_cli = "AWS CLI - https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html"
    terraform_cli = "Terraform - https://developer.hashicorp.com/terraform/downloads"
    if command in {None, "bootstrap", "terraform", "full"} or (command == "runners" and not is_gcp_runners):
        required.update({"aws": aws_cli, "terraform": terraform_cli})

    # GCP runners use gcloud + ADC (no AWS CLI); still need terraform + gh.
    if is_gcp_runners:
        required.update(
            {
...
Examples:
  ...
  ./scripts/bootstrap/deploy.py gdc-bootstrap --project-id prod-rwctxzl6shxk --cluster-id cluster1
    ...
    # Runners command (issue #1433 AWS; issue #1546 GCP): provision + auto-register
    # self-hosted runners. --cloud selects the provider; AWS is the default for
    # back-compat. GCP provisions into the target GCP project (dev-tenant
    # containment) with a dedicated runner VPC and IAP-only registration.
...
def main() -> None:
    ...
    if args.command == "bootstrap":
        ...
        if args.cloud == Cloud.GCP.value:
            # The dedicated runner network is mandatory for GCP (ADR-008-R8), so
            # the AWS-only existing-network opt-out is rejected rather than
            # silently ignored.
            if args.use_existing_network:
                error(
                    "--use-existing-network is not supported for --cloud gcp; the dedicated runner network is mandatory"
                )
                sys.exit(1)
            # gcp_runner._verify_prerequisites fails closed on gh auth + gcloud
            # ADC before any mutation. The full platform preflight_gate is
            # intentionally not used here: a runner standup must not require the
            # platform environment's secret/config surface.
