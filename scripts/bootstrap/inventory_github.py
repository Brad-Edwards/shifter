...
def verify_github_actor(record: DeploymentRecord, actor: str, *, inventory_repository: str | None = None) -> None:
    ...
    repository = private_json(["gh", "api", "repos/" + record.execution.repository])
    if (
    ...
    if inventory_repository and inventory_repository != record.execution.repository:
        inventory = private_json(["gh", "api", "repos/" + inventory_repository])
        if (
            inventory.get("private") is not True
            or inventory.get("full_name") != inventory_repository
...
def _branch_policies(endpoint: str) -> list[dict]:
...
def _preserved_protections(current: dict) -> dict:
...
def reconcile_environments(record: DeploymentRecord, *, apply: bool) -> None:
    ...
    """
    repo = "repos/" + record.execution.repository
