...
_ORIGINAL_EXPORTS: dict[str, tuple[ModuleType, Any]] = {}
...
def _should_export_from_owner(name: str, value: Any, module: ModuleType) -> bool:
    if name.startswith("__"):
        return False
...
def _make_facade(name: str, module: ModuleType, original: Callable[..., Any]) -> Callable[..., Any]:
...
def _unwrapped_export(name: str, current: Any, original: Any) -> Any:
...
def _sync_modules() -> None:
    for name, (_owner, original) in _ORIGINAL_EXPORTS.items():
        value = _unwrapped_export(name, globals().get(name, original), original)
...
main: Callable[..., Any] = globals()["main"]
