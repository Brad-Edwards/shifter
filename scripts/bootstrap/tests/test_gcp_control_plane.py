...
class TestGdcBootstrapRangeBackend:
    ...
    def test_gce_backend_skips_substrate_and_deploys_control_plane(self):
        ...
        # Satisfy the range-precondition gate (#1509) for real rather than patching
        # the first-party gate function: ADR-019-R1 forbids patching first-party
        # internals. The required range vars are present and the gcloud image lookup
        # (the real process boundary) reports the referenced images exist, so
        # check_gce_range_preconditions passes and the deploy proceeds. The gate
        # itself is exercised directly in TestGceRangePreconditions below.
        passing_range_env = {
        ...
        with (
            patch.dict(os.environ, passing_range_env, clear=False),
            patch(
        ...
        # Reaching the deploy proves the gce path passed the range-precondition gate
        # (#1509) before any mutation and skipped the GDC substrate entirely.
