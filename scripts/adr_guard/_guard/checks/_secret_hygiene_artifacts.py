...
#
# Scenario build output and operator-run state are generated material. Scope
# these rules by layout and artifact name, never a private pack identity.
...
def _is_operator_run_artifact(basename: str) -> bool:
    """Return True for tracked operator run outputs."""
    return basename in (
...
_GENERATED_ARTIFACT_PREDICATES: tuple[tuple[str, Callable[[str], bool]], ...] = (
    ...
    ("scripts/", _is_operator_run_artifact),
...
def _generated_artifact_match(rel_path: str) -> bool:
    ...
    if len(parts) >= 4 and parts[0] == "scenario-dev" and parts[2] == "build":
        return True
    basename = parts[-1]
    return any(
...
def check_no_tracked_generated_artifacts(
    ...
    - Scenario build output under `scenario-dev/<pack>/build/`, and operator
      run state or reports under `scripts/`.
