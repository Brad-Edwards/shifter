...
def test_dns_resolver_tf_defines_split_horizon_firewall() -> None:
    ...
    # Each rule's action must be checked against the domain list it actually
    # references. Asserting "BLOCK" and "block_unlisted" appear somewhere in the
    # file passes even if the allowed and catch-all list references were
    # swapped - which would invert the control, blocking the scenario and
    # bootstrap domains while allowing everything else (#1172).
