#!/usr/bin/env python3
"""Ensure blocking CI Checkov policy and parity for any optional local hook.
...
download_external_modules. A local hook is optional; if installed, it must match.
"""
...
def check_repo(repo_root: Path) -> list[str]:
    ...
    # Absence is intentional for fast commits; malformed existing hooks still fail.
    if re.search(r"(?m)^\s*-\s+id:\s+checkov\s*$", precommit_text):
