...
- **Evidence status**: `captured` only when the row cites an **immutable
  ...
  local runs and this gate's own routed Quality run are non-authoritative
  confirmation that the producers are green at the current tree, never the
  capture basis. A row lacking an immutable locator is `not-yet-demonstrated`
  (or `live-manual`), and the final column states the owned residual limitation
  and what a live capture would require.
