...
| Runtime storage adapter | `shared.cloud.get_object_storage()`, `shared.cloud.aws.storage`, `cms.assets.s3`, `cms.experiments.s3`, `ctf.s3` | Runtime verification or cleanup must reuse the object-storage protocol and sanitizers, not raw boto3 logic scattered through domain services. |
