...
1. **Self-asserted identity can grant CTF organizer authority.** A user-controlled
   `custom:user_type` value maps to the global organizer group, which can create
   and administer events and trigger downstream provisioning. This contradicts
   the invariant accepted when issue #937 closed.
