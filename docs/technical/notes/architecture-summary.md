...
|---------|---------------|
| **RDS PostgreSQL** | db.t3.large, v16, 20-100 GB autoscaling, 7-day backups, log exports enabled |
| **Redis (ElastiCache)** | cache.t3.micro, v7.1, replication group enabled (2 nodes, multi-AZ, automatic failover) |
| **S3** | User storage bucket for agent uploads (5 GB/user quota, 2 GB/file limit) |
