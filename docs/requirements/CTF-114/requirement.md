...
The system should enforce a configurable rate limit on flag submissions per participant per challenge (e.g. no more than 1 submission per 10 seconds). Rate-limited submissions shall be rejected with a clear message indicating when the participant may retry. Rate limit parameters shall be configurable per event.
