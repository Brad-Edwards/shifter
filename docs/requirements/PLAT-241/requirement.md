...
- IMPLEMENTS → CODE_FILE `shifter/shifter_platform/workspaces/services/_invitations.py` (Provider-neutral invitation domain service reusing Django signing, shared email, shared audit, and the workspace role seam)
- IMPLEMENTS → CODE_FILE `shifter/shifter_platform/workspaces/api/invitation_views.py` (Session-only DRF invitation boundary with live workspace-operation authorization)
- IMPLEMENTS → CODE_FILE `shifter/shifter_platform/workspaces/admin.py` (Read-only Django-admin invitation escape hatch)
- IMPLEMENTS → CODE_FILE `shifter/shifter_platform/frontend/src/api/invitations.ts` (Shared apiFetch, generated OpenAPI types, and TanStack Query invitation data layer with no cloud branch)
- IMPLEMENTS → CODE_FILE `shifter/shifter_platform/frontend/src/features/administer/organization/WorkspaceInvitationsPage.tsx` (Cloud-agnostic administration UI driven by server-advertised operations)
- TESTS → TEST `shifter/shifter_platform/tests/workspaces/test_invitation_api.py` (Session-only DRF admission and workspace authorization coverage)
- TESTS → TEST `shifter/shifter_platform/tests/workspaces/test_invitations.py` (Proves reuse of shared signing, email, audit, role, and membership seams)
- TESTS → TEST `shifter/shifter_platform/frontend/src/features/administer/organization/WorkspaceInvitationsPage.test.tsx` (Cloud-neutral generated-contract administration surface coverage)
