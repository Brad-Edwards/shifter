...
- Only `shared` may import `cyberscript` directly; all other `shifter_platform` layers must use `shared` shims. New non-DSL contracts belong in `shared` natively — `cyberscript` is for scenario DSL contracts only and should shrink over time ahead of the RAES swap.
